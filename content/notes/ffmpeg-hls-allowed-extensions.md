...
aliases: ["/wiki/ffmpeg-hls-allowed-extensions/"]
---
...
[[ffmpeg]]의 [[hls]] 디먹서는 **세그먼트 확장자 화이트리스트**를 가진다. `.m4v` / `.m4s` 같은
...
같이 필요했던 것: CDN이 ffmpeg 기본 [[user-agent|User-Agent]]를 거부해서 `-user_agent`도 항상 넣는다.
